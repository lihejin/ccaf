...
import com.ccaf.service.GeneratorService;
import com.ccaf.util.GenUtils;
...
public class GeneratorController {
	...
	public void code(HttpServletRequest request, HttpServletResponse response,
		...
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\"newtouch.zip\"");
	...
	public void batchCode(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String[] tableNames = request.getParameter("tables").split(",");
		byte[] data = generatorService.generatorCode(tableNames);
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\"newtouch.zip\"");
		response.addHeader("Content-Length", "" + data.length);
